...
def test_format_src_trivial():
    after, _ = Formatter(mode=BLACK_MODE).process_str("")
    assert after == ""
...
def test_format_src_markdown_simple():
    ...
    after, _ = Formatter(mode=BLACK_MODE).process_str(before)
    assert after == dedent(
        """\
        ```python
        f(1, 2, 3)
...
def test_format_src_markdown_python_after_newline():
    ...
    after, errors = Formatter(mode=BLACK_MODE).process_str(before)
    assert errors == []
    assert after == before
...
def test_format_src_rst_literal_blocks_nested():
    ...
    after, errors = Formatter(mode=BLACK_MODE).process_str(
        before,
        rst_literal_blocks=True,
    )
    assert after == before
...
def test_format_src_rst_literal_blocks_comments():
    ...
    after, _ = Formatter(mode=BLACK_MODE).process_str(before, rst_literal_blocks=True)
    assert after == before
